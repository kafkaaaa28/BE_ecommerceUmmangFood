...
function hashOtp(identifier: string, otp: string) {
  return crypto.createHash('sha256').update(`${OTP_PEPPER}:${identifier}:${otp}`).digest('hex');
}
...
export class OtpService {
  async sendOtp(email: string, purpose: cache.OtpPurpose, identifier?: string) {
    ...
    console.log(`OTP for ${email} (purpose: ${purpose}, identifier: ${otpIdentifier}): ${otp}`);
  }

  async verifyOtp(identifier: string, otp: string, purpose: cache.OtpPurpose): Promise<void> {
    const candidate = hashOtp(identifier, otp);
    const result = await cache.consumeOtpIfMatch(identifier, purpose, candidate);
